...
public class ConflictResolutionTests
{
    ...
    public void NoChangeConflict()
    {
        ...
        s_resolver.TryAutoResolveConflicts(resolution, new RefSigner(s_algorithm, s_new.PrivateInfo), out ValidatedVaultDataSnapshot snapshot)
            .Should()
            .BeTrue();
        snapshot.TryGetSignerPublicInfo(out PublicKeyInfo signerInfo).Should().BeTrue();
        signerInfo.Id.Should().Be(s_self.Id, "no conflict should not resign/modify vault");
    }
    ...
    public void DualAddsAreMergedOnlyModification()
    {
        ...
        s_resolver.TryAutoResolveConflicts(conflict, new RefSigner(s_algorithm, s_new.PrivateInfo), out ValidatedVaultDataSnapshot snapshot)
            .Should()
            .BeTrue();
        snapshot.TryGetSignerPublicInfo(out PublicKeyInfo signerInfo).Should().BeTrue();
        signerInfo.Id.Should().Be(s_self.Id, "no conflict should resign/modify vault");
    ...
    public void ConflictingModificationsReportsConflict()
    {
        ...
        SealedSecret<SecretAttributes, SecretProtectedValue> baseSecret =
            conflictItem.BaseEntry.Should().BeOfType<SealedSecret<SecretAttributes, SecretProtectedValue>>().Subject;
        conflictItem.Local.Is(out UntypedSealedSecret localUntyped).Should().BeTrue();
        SealedSecret<SecretAttributes, SecretProtectedValue> localSecret = localUntyped.Should()
            ...
            .Subject;
        conflictItem.Remote.Is(out UntypedSealedSecret remoteUntyped).Should().BeTrue();
    ...
    public void ConflictingModificationsDefaultResolveLocal()
    {
        ...
        VaultConflictResult conflict = s_resolver.Resolve(original, modifiedRemoteVault, modifiedLocalVault);
        PartialVaultConflictResolution resolver = conflict.GetResolver();
        resolver.Apply(s_algorithm, s_self.PrivateInfo, VaultResolutionItem.AcceptLocal)
            .TryGetValue(out ValidatedVaultDataSnapshot resolvedVault)
            .Should()
            .BeTrue();
    ...
    public void ConflictingModificationsDefaultResolveRemote()
    {
        ...
        s_resolver.TryAutoResolveConflicts(conflict, new RefSigner(s_algorithm, s_new.PrivateInfo), out _).Should().BeFalse();
        PartialVaultConflictResolution resolver = conflict.GetResolver();
        resolver.Apply(s_algorithm, s_self.PrivateInfo, VaultResolutionItem.AcceptRemote)
            .TryGetValue(out ValidatedVaultDataSnapshot resolvedVault)
            .Should()
            .BeTrue();
        (string attribute, string prot) = GetSecretValues(resolvedVault, id);
    ...
    public void DifferentResolutionsResolvedTogether()
    {
        ...
        {
            (iterationCount++).Should().BeLessThanOrEqualTo(2, "should only have two conflicts that are resolved");
            ...
            else if (secretConflict.BaseEntry.Id == id2) resolver = resolver.WithResolution(unresolved, VaultResolutionItem.AcceptRemote);
        }
    ...
    {
        return AddSecret(ref input, new SecretAttributes { Value = attributeValue }, new SecretProtectedValue { ProtValue = protectedValue }, client);
    }
    ...
    {
        return AddSecret(ref input, VaultIdentifier.Create<TAttribute, TProtected>("First Vault"), attributeValue, protectedValue, client);
    }
    ...
    {
        return GetSecretValues(snapshot, VaultIdentifier.Create<SecretAttributes, SecretProtectedValue>("First Vault"), secretId);
    }
    ...
    {
        UnsealedSecret<SecretAttributes, SecretProtectedValue> secret =
            s_transformer.Unseal(GetSecret<SecretAttributes, SecretProtectedValue>(snapshot, vaultId, secretId));
        return (secret.Attributes.Value, secret.Protected.ProtValue);
    }

    private SealedSecret<TAttributes, TProtected> GetSecret<TAttributes, TProtected>(
        ValidatedVaultDataSnapshot snapshot,
        VaultIdentifier vaultId,
        Guid secretId
    )
    ...
    private class ClientData
    {
        ...
        public PrivateKeyInfo PrivateInfo { get; }
        public BlockedVaultClientEntry Blocked { get; }

        public PublicKeyInfo PublicInfo => Entry.PublicInfo;

        public static ClientData Create(Guid id, string description, ClientData signClient = null)
        {
            PrivateKeyInfo privateInfo;
            PublicKeyInfo publicInfo;
            if (signClient == null)
            {
                s_algorithm.CreateKeys(id, out privateInfo, out publicInfo);
            }
            else
            {
            ...
            VaultClientEntry entry =
                new VaultManager(s_transformer, s_algorithm, new LiveVaultData()).ApproveRequest(new RefSigner(s_algorithm, privateInfo), request);
