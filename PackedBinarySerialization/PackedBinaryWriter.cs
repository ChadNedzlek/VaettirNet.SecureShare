...
#nullable disable
    ...
    {
        if (typeof(T) == typeof(void)) return 0;

        if (typeof(T) == typeof(byte)) return writer.WriteByte(ReflectionHelpers.As<T, byte>(value), ctx);

        if (typeof(T) == typeof(sbyte)) return writer.WriteSByte(ReflectionHelpers.As<T, sbyte>(value), ctx);

        if (typeof(T) == typeof(short)) return writer.WriteInt16(ReflectionHelpers.As<T, short>(value), ctx);

        if (typeof(T) == typeof(ushort)) return writer.WriteUInt16(ReflectionHelpers.As<T, ushort>(value), ctx);

        if (typeof(T) == typeof(int)) return writer.WriteInt32(ReflectionHelpers.As<T, int>(value), ctx);

        if (typeof(T) == typeof(uint)) return writer.WriteUInt32(ReflectionHelpers.As<T, uint>(value), ctx);

        if (typeof(T) == typeof(long)) return writer.WriteInt64(ReflectionHelpers.As<T, long>(value), ctx);

        if (typeof(T) == typeof(ulong)) return writer.WriteUInt64(ReflectionHelpers.As<T, ulong>(value), ctx);

        if (typeof(T) == typeof(float)) return writer.WriteSingle(ReflectionHelpers.As<T, float>(value), ctx);

        if (typeof(T) == typeof(double)) return writer.WriteDouble(ReflectionHelpers.As<T, double>(value), ctx);

        if (typeof(T) == typeof(string)) return writer.WriteString(ReflectionHelpers.As<T, string>(value), ctx);

        if (typeof(T) == typeof(bool)) return writer.WriteBool(ReflectionHelpers.As<T, bool>(value), ctx);

        if (typeof(T) == typeof(char)) return writer.WriteChar(ReflectionHelpers.As<T, char>(value), ctx);

        if (typeof(T) == typeof(Guid)) return writer.WriteGuid(ReflectionHelpers.As<T, Guid>(value), ctx);

        if (typeof(T).IsEnum) return writer.WriteEnum(value, ctx);
        ...
        if (!typeof(T).IsValueType) return writer.WriteRefValue<T>(value, ctx);
    ...
    private int WriteRefValue<T>(object value, PackedBinarySerializationContext ctx)
    {
        ...
        if (TryWriteWithMetadata(refT, ctx, out written)) return written;
    ...
    private static int WriteSerializable(
        ref PackedBinaryWriter<TWriter> writer,
        IPackedBinarySerializable value,
        PackedBinarySerializationContext ctx
    )
    {
    ...
    public int WriteSerializable<T>(T value, PackedBinarySerializationContext ctx)
        where T : IPackedBinarySerializable
    {
        int written = 0;
    ...
    private bool TryWriteSerializable<T>(object value, PackedBinarySerializationContext ctx, out int written)
    {
        ...
        {
            written = s_serializableReflector.GetSerializer<IPackedBinarySerializable>(typeof(T))
                .Invoke(ref this, (IPackedBinarySerializable)value, ctx);
            return true;
        }
